...
class PositionBasedSummarizer:
    ...
    def position_based_summary(self, text, num_sentences=5):
        """Generate position-based summary combining position and content importance"""
        if not text:
            return []
        ...
        # Calculate word frequencies for content scoring
        ...
        for i, sentence in enumerate(sentences):
            ...
            # Content score based on word frequency
            sentence_words = self.word_tokenize_and_clean(sentence)
            content_score = sum(word_freq[word] for word in sentence_words)
            content_score = content_score / len(sentence_words) if sentence_words else 0
            
            # Length bonus for moderate-length sentences
            length_score = min(len(sentence_words) / 20, 1.0)
            
            # Combined score
            sentence_scores[sentence] = position_score * content_score * (1 + length_score)
    ...
    def process_file(self, file_path):
        ...
        # Generate summary
        try:
            summary_sentences = self.position_based_summary(cleaned_text)
    ...
    def show_summary(self, summary_sentences, filename):
        ...
        summary_window.title(f"Position-based Summary - {filename}")
        summary_window.geometry("800x600")
        summary_window.configure(bg='#fff3e0')
        
        # Create main frame
        main_frame = tk.Frame(summary_window, bg='#fff3e0')
        main_frame.pack(fill=tk.BOTH, expand=True, padx=20, pady=20)
        
        # Title
        title_label = tk.Label(main_frame, 
                              text="POSITION-BASED SUMMARY", 
                              font=('Arial', 16, 'bold'),
                              bg='#fff3e0', fg='#e65100')
        ...
        info_label = tk.Label(main_frame, 
                             text=f"Source: {filename} | Method: Position + Content | Sentences: {len(summary_sentences)}", 
                             font=('Arial', 10),
                             bg='#fff3e0', fg='#f57c00')
        info_label.pack(pady=(0, 15))
        
        # Summary text area
        text_frame = tk.Frame(main_frame, bg='#fff3e0')
    ...
    def save_summary(self, original_filename):
        ...
        suggested_name = f"{base_name}_position_summary.txt"
        ...
        if file_path:
            try:
                with open(file_path, 'w', encoding='utf-8') as file:
                    file.write(f"POSITION-BASED SUMMARY\n")
    ...
    def create_gui(self):
        """Create the main GUI"""
        self.root = tk.Tk()
        self.root.title("Position-based Text Summarizer")
        self.root.geometry("600x400")
        self.root.configure(bg='#fbe9e7')
        
        # Main container
        main_container = tk.Frame(self.root, bg='#fbe9e7')
        main_container.pack(fill=tk.BOTH, expand=True, padx=30, pady=30)
        
        # Title
        title_label = tk.Label(main_container, 
                              text="POSITION-BASED TEXT SUMMARIZER", 
                              font=('Arial', 20, 'bold'),
                              bg='#fbe9e7', fg='#d84315')
        title_label.pack(pady=(0, 10))
        ...
        subtitle_label = tk.Label(main_container, 
                                 text="Prioritizes sentences from beginning and end with content analysis", 
                                 font=('Arial', 12),
                                 bg='#fbe9e7', fg='#f4511e')
...
🎯 HOW IT WORKS:
• Gives higher scores to sentences at document beginning and end
• Combines positional importance with word frequency analysis
• Assumes key information appears in introduction and conclusion
• Balances structural and content-based importance
...
4. Process another document or press ESC to exit
        ...
        # Buttons frame
        buttons_frame = tk.Frame(main_container, bg='#fbe9e7')
...
if __name__ == "__main__":
    ...
    print("Position-based Text Summarizer")
    print("="*40)
